...
public class HttpclientProxy {
    ...
    public static String execPOSTMethodMParames(String url, Map<String, String> parames)
        ...
        if (parames.size() > 0) {
            ...
            int parameLenth = 0;
            for (Map.Entry<String, String> entry : parames.entrySet()) {
                data[parameLenth] = new NameValuePair(entry.getKey(), entry.getValue());
                parameLenth += 1;
            }
    ...
    public static String execPOSTMethodJSONSoTimeout(String url, String json, int soTimeout, int conTimeout,
        ...
        // 执行postMethod
        int statusCode = httpClient.executeMethod(postMethod);
        log.info("url:{},headers:{},http status:{} ", url, headers, statusCode);
        if (statusCode != HttpStatus.SC_OK) {
            log.info("Method failed: " + postMethod.getStatusLine());
            log.info(new String(postMethod.getResponseBody()));
    ...
    public static String execGETMethodJSON(String url, List<Header> headers) throws HttpException, IOException {
        return execGETMethodJSONSoTimeout(url, 30000, 30000, headers);
    }
